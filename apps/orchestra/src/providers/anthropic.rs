//! Anthropic provider — wraps the Claude Code CLI subprocess.
//!
...
impl StepProvider for AnthropicProvider {
    async fn execute(
        ...
        let worktree = task
            .working_dir
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("AnthropicProvider requires working_dir"))?;
        let log_file = task
            .log_file
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("AnthropicProvider requires log_file"))?;
