    ...
    <div class="p-3 sm:p-6 max-w-3xl mx-auto" *transloco="let t">
      ...
      @if (currentStep() === 'form') {
        <div class="bg-surface rounded-lg border border-border p-6">
          <h2 class="text-lg font-medium text-text-primary mb-1">{{ t('github.connectTitle') }}</h2>
      ...
      @if (currentStep() === 'webhook') {
        <div class="bg-surface rounded-lg border border-border p-6">
          <h2 class="text-lg font-medium text-text-primary mb-1">{{ t('github.webhookTitle') }}</h2>
          <p class="text-sm text-text-secondary mb-6">{{ t('github.webhookDescription') }}</p>

          <!-- Instructions -->
          <div class="bg-bg-subtle rounded-lg p-4 mb-6 border border-border">
            <h3 class="text-sm font-semibold text-text-primary mb-3">{{ t('github.webhookInstructions') }}</h3>
            <ol class="space-y-2 text-sm text-text-secondary list-decimal list-inside">
              <li>{{ t('github.webhookStep1') }}</li>
              <li>{{ t('github.webhookStep2') }}</li>
              <li>{{ t('github.webhookStep3') }}</li>
              <li>{{ t('github.webhookStep4') }}</li>
              <li>{{ t('github.webhookStep5') }}</li>
            </ol>
          </div>

          <!-- Webhook URL -->
          <div class="mb-4">
            <span class="block text-sm font-medium text-text-secondary mb-1">{{ t('github.webhookUrl') }}</span>
            <div class="flex items-center gap-2">
          ...
          <!-- Webhook Secret -->
          <div class="mb-6">
            <span class="block text-sm font-medium text-text-secondary mb-1">{{ t('github.webhookSecret') }}</span>
            <div class="flex items-center gap-2">
              <code class="flex-1 bg-bg-subtle text-text-primary text-sm rounded-lg px-3 py-2 border border-border font-mono break-all">
                @if (showSecret()) {
              ...
              <button (click)="showSecret.set(!showSecret())"
                class="shrink-0 px-3 py-2 text-sm font-medium bg-surface border border-border rounded-lg hover:border-accent/50 transition-colors">
                {{ showSecret() ? t('github.hide') : t('github.show') }}
              </button>
            ...
            <p class="text-xs text-ctp-yellow mt-1">{{ t('github.secretWarning') }}</p>
          </div>
      ...
      @if (currentStep() === 'sync') {
        <div class="bg-surface rounded-lg border border-border p-6">
          <h2 class="text-lg font-medium text-text-primary mb-1">{{ t('github.syncTitle') }}</h2>
          <p class="text-sm text-text-secondary mb-6">{{ t('github.syncDescription') }}</p>
          ...
          <div class="flex items-center gap-4 mb-6">
            ...
            @if (syncResult()) {
              <span class="text-sm text-ctp-green">
                {{ t('github.syncResult', { synced: syncResult()!.synced, errors: syncResult()!.errors }) }}
              </span>
            }
          </div>
          ...
          <div class="bg-ctp-green/5 border border-ctp-green/20 rounded-lg p-4 mb-6">
            <div class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm font-medium text-text-primary">{{ t('github.setupComplete') }}</p>
                <p class="text-sm text-text-secondary mt-1">{{ t('github.setupCompleteHint') }}</p>
              </div>
            </div>
          </div>
...
export class GitHubSetupPage {
  ...
  integration = signal<GitHubIntegrationResponse | null>(null);
  ...
  syncRuns(): void {
    ...
    this.ciApi.syncGitHub(pid).subscribe({
      next: (result) => {
