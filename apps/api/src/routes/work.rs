...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/{project_id}/work/{work_id}/apply-plan", post(apply_plan))
        .route(
...
/// Batch-create tasks from a plan and wire their dependencies atomically.
///
/// All tasks are created in a single database transaction so the orchestra
/// never sees a task in "ready" state before its dependency edges exist.
/// This prevents race conditions where agents pick up dependent tasks before
/// their blockers are registered.
async fn apply_plan(
    State(state): State<AppState>,
    AuthUser(user_id): AuthUser,
    OptionalAgentId(agent_id): OptionalAgentId,
...
) -> Result<Json<ApplyPlanResponse>, AppError> {
    ...
    if req.tasks.is_empty() {
        return Ok(Json(ApplyPlanResponse {
            tasks: vec![],
            dependencies: vec![],
        }));
    }
    ...
    if req.tasks.len() > 20 {
        return Err(AppError::Validation(
            "Plan cannot contain more than 20 tasks".into(),
        ));
    }

    // Get project for default playbook
    let project = state.db.get_project_by_id(project_id).await?;
    let playbook_id = project.default_playbook_id;

    // Determine target initial state from playbook
    let target_state = if let Some(pb_id) = playbook_id {
        let pb = state.db.get_playbook_by_id(pb_id).await?;
        pb.initial_state.clone()
    } else {
        "backlog".to_string()
    };

    // Use a transaction to create all tasks + dependencies atomically.
    // Tasks are created in "backlog" first, dependencies are wired, then
    // tasks are promoted to the target state — all within one commit.
    let mut tx = state
        .pool
        .begin()
        .await
    ...
    // Step 1: Create all tasks in backlog state
    for planned in &req.tasks {
        ...
        let task = sqlx::query_as::<_, Task>(
            "INSERT INTO diraigent.task \
             (project_id, title, kind, state, urgent, context, required_capabilities, \
              playbook_id, playbook_step, created_by, file_scope, state_entered_at) \
             VALUES ($1, $2, $3, 'backlog', false, $4, '{}', $5, $6, $7, '{}', now()) \
             RETURNING *",
        )
        .bind(project_id)
        .bind(&planned.title)
        ...
        // Link to work item
        sqlx::query(
            "INSERT INTO diraigent.task_work (work_id, task_id) \
    ...
    // Step 2: Wire dependencies using depends_on indices
    for (i, planned) in req.tasks.iter().enumerate() {
        if let Some(deps) = &planned.depends_on {
            for &dep_idx in deps {
                if dep_idx < created_tasks.len() && dep_idx != i {
                    let dep = sqlx::query_as::<_, TaskDependency>(
                        "INSERT INTO diraigent.task_dependency (task_id, depends_on) \
                         VALUES ($1, $2) RETURNING *",
                    )
                    .bind(created_tasks[i].id)
                    .bind(created_tasks[dep_idx].id)
                    .fetch_one(&mut *tx)
                    .await
                    .map_err(|e| match &e {
                        sqlx::Error::Database(db) if db.constraint().is_some() => {
                            AppError::Conflict(
                                "Dependency already exists or invalid reference".into(),
                            )
                        }
                        _ => e.into(),
                    })?;
    ...
    // Step 3: Promote all tasks from backlog to target state
    if target_state != "backlog" {
        for task in &mut created_tasks {
            let updated = sqlx::query_as::<_, Task>(
                "UPDATE diraigent.task \
                 SET state = $2, state_entered_at = now() \
    ...
    // Commit — orchestra only sees ready tasks with dependencies already wired
    tx.commit()
        .await
        .map_err(|e| AppError::Internal(format!("Failed to commit transaction: {e}")))?;

    // Fire events for each created task (after commit, outside transaction)
    ...
    // Refresh auto-status for all created tasks
    for task in &created_tasks {
        refresh_auto_status_works(&state, task.id, agent_id).await;
    }
