...
pub async fn get_scratchpad(
    ...
    Ok(sqlx::query_as::<_, Scratchpad>(
        "SELECT * FROM diraigent.scratchpad WHERE user_id = $1 AND project_id = $2",
    )
    .bind(user_id)
    .bind(project_id)
    .fetch_optional(pool)
...
pub async fn upsert_scratchpad(
    ...
    Ok(sqlx::query_as::<_, Scratchpad>(
        "INSERT INTO diraigent.scratchpad (user_id, project_id, notes, todos, updated_at)
         VALUES ($1, $2, $3, $4, now())
